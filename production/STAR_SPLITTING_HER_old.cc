  ...
  public:
    ...
    */
	...
	_phibins.push_back(phix);	
      ...
      */
      ...
      if(_mode == 0){
	fout = new TFile(Form(/*"Results/py6_decayed_jewel_pthatbin580_R0%d.root"*/"Results/star_subjetvars_herwig7_5pthatbin10_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }
      else if(_mode == 1){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_10pthatbin15_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 2){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_15pthatbin20_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 3){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_20pthatbin25_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 4){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_25pthatbin30_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 5){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_30pthatbin35_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 6){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_35pthatbin40_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 7){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_40pthatbin45_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 8){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_45pthatbin50_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 9){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_50pthatbin60_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }else if(_mode == 10){
	fout = new TFile(Form("Results/star_subjetvars_herwig7_60pthatbin80_R0%d_decays_off_test.root", RADIUS),"RECREATE");
	fout->cd();
      }
	...
	*/
	...
	for (int i = 0; i < (int) hardparton.size(); ++ i) {
	  if (jet.delta_R(hardparton[i]) < _jetR) {
	    match = 1;
	...
	if (match == 0) {qvg.push_back(-1);} //no match!
