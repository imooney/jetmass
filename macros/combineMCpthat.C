...
double pthatweight_PY8[npthatbins+1] = {1570.48, 9.59444, 0.620145, 0.0780124, 0.0132472, 0.00261879, 0.000563622, 0.000126134, 2.85752e-05, 7.73407e-06, 8.76938e-07};//{446225, 184.244, 3.35316, 0.23586, 0.0293411, 0.00475607, 0.000888898, 0.000177778, 3.6652e-05, 9.05141e-06, 9.21373e-07};
...
void combineMCpthat(std::string mctype = "pythia8", int radius = 4)
{
  ...
  TFile * fout = new TFile(Form("Results/%s_recSD_histograms_R0%d.root", mctype.c_str(), radius),"RECREATE");
  fout->cd();
  ...
  std::string fileloc = "/nfs/rhi/STAR/Data/RecJetStructure/ppPYTHIA8Gen";
  ...
  for(unsigned inpt = 0; inpt < npthatbins; ++inpt){
    ...
    std::string pthatbinname = Form("star_subjetvars_%s_%dpthatbin%d_R0%d.root",
				    mctype.c_str(),
				    pthatbins[inpt],
				    pthatbins[inpt+1],
				    radius
