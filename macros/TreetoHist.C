...
void HistsFromTree(TFile* file, std::vector<TH1D*> hists1D, std::vector<TH2D*> hists2D, const bool dataflag) {
  ...
  vector<double> *ptg = 0; vector<double> *E = 0; vector<double> *ch_e_frac = 0;
  ...
  for (int i = 0; i < t->GetEntries(); ++ i) {
	...
	hists1D[11]->Fill(ptg->at(j) / (double) Pt->at(j));
      ...
      //inclusive > 15 GeV (1D)
      /*      if (Pt->at(j) >= 15) {
	hists1D[12]->Fill(Pt->at(j), weight);
	hists1D[13]->Fill(Eta->at(j), weight);
	hists1D[14]->Fill(Phi->at(j), weight);
	hists1D[15]->Fill(E->at(j), weight);
	hists1D[16]->Fill(M->at(j), weight);
	hists1D[17]->Fill(ch_e_frac->at(j), weight);
	if (zg->at(j) >= 0.1) {
	  hists1D[18]->Fill(zg->at(j), weight);
	  hists1D[19]->Fill(rg->at(j), weight);
	  hists1D[20]->Fill(mg->at(j), weight);
	  hists1D[21]->Fill(ptg->at(j), weight);
	  hists1D[22]->Fill(ptg->at(j) / (double) Pt->at(j));
	}
	}*/
      //Filling 2Ds!
...
void TreetoHist() {
  ...
  TFile* pyFile = new TFile( (dir + pyin +"test.root").c_str(), "READ");
  ...
  TH1D* ratio_ptg_pt_d = new TH1D("ratio_ptg_pt_d","",20,0,20);
  TH1D* ratio_ptg_pt_g = new TH1D("ratio_ptg_pt_g","",20,0,20);
  TH1D* ratio_ptg_pt_p = new TH1D("ratio_ptg_pt_p","",20,0,20);
  ...
  */
  ...
  TH2D* m_v_pt_p = new TH2D("m_v_pt_p","",20,0,10,9,15,60);
  ...
  vector<TH1D*> d_hists1D = {pt_d_var_bin,pt_d,eta_d,phi_d,e_d,m_d,ch_e_frac_d,zg_d,rg_d,mg_d,ptg_d,ratio_ptg_pt_d,/*pt_d_pt15,eta_d_pt15,phi_d_pt15,e_d_pt15,m_d_pt15,ch_e_frac_d_pt15,zg_d_pt15,rg_d_pt15,mg_d_pt15,ptg_d_pt15,ratio_ptg_pt_d_pt15*/};
  vector<TH2D*> d_hists2D = {m_v_pt_d,ch_frac_v_pt_d,zg_v_pt_d,rg_v_pt_d,mg_v_pt_d,ptg_v_pt_d};

  vector<TH1D*> g_hists1D = {pt_g_var_bin,pt_g,eta_g,phi_g,e_g,m_g,ch_e_frac_g,zg_g,rg_g,mg_g,ptg_g,ratio_ptg_pt_g,/*pt_g_pt15,eta_g_pt15,phi_g_pt15,e_g_pt15,m_g_pt15,ch_e_frac_g_pt15,zg_g_pt15,rg_g_pt15,mg_g_pt15,ptg_g_pt15,ratio_ptg_pt_g_pt15*/};
  vector<TH2D*> g_hists2D = {m_v_pt_g,ch_frac_v_pt_g,zg_v_pt_g,rg_v_pt_g,mg_v_pt_g,ptg_v_pt_g};
  
  vector<TH1D*> p_hists1D = {pt_p_var_bin,pt_p,eta_p,phi_p,e_p,m_p,ch_e_frac_p,zg_p,rg_p,mg_p,ptg_p,ratio_ptg_pt_p,/*pt_p_pt15,eta_p_pt15,phi_p_pt15,e_p_pt15,m_p_pt15,ch_e_frac_p_pt15,zg_p_pt15,rg_p_pt15,mg_p_pt15,ptg_p_pt15,ratio_ptg_pt_p_pt15*/};
  vector<TH2D*> p_hists2D = {m_v_pt_p,ch_frac_v_pt_p,zg_v_pt_p,rg_v_pt_p,mg_v_pt_p,ptg_v_pt_p};
