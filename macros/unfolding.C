...
void UnfoldedObs(TFile* matchFile, TH1D* raw, TH1D* gen, TH1D* det/*, TH1D* raw15*/, const string resName, const string xTitle, const string log, const string out, const string filetype, const string flag) {
  ...
  TH1D* hReFolded = (TH1D*) res->ApplyToTruth(reco2);
  ...
  TH1D* ratio = (TH1D*) hReFolded->Clone("ratio");
  ratio->SetTitle("");
  
  Prettify1D(ratio, kBlack, kOpenCircle, 2, kBlack, xTitle, "refolded / raw",-1,-1,0.5,1.5);
  
  ratio->Divide(raw);
  
  TCanvas *cr = MakeCanvas("cr","0",1400,1000); cr->cd();
  
  TLegend *t = TitleLegend(0.15,0.15, 0.45, 0.35);
  t->AddEntry((TObject*)0,"Bayesian refolding test","");
 
  cout << ratio->GetBinContent(3) << " " << raw->GetBinContent(3) << endl;

  TLine *five = new TLine(0,1.05,10,1.05); five->SetLineStyle(kDashed);
  TLine *minfive = new TLine(0,0.95,10,0.95); minfive->SetLineStyle(kDashed);
  TLine *one = new TLine(0,1,10,1); one->SetLineStyle(kDashed);
  
  ratio->Draw(); raw->Draw("same"); five->Draw("same"); minfive->Draw("same"); one->Draw("same"); t->Draw("same");
  //  ratio->Draw();
  
  cr->SaveAs((out+"refolding_ratio_"+raw->GetName() + filetype).c_str());
...
void SliceUnfolded4D(TFile *matchFile, TH2D* raw, TH2D* gen, TH2D* det, const string log, const string out, const string filetype, const string xTitle, const string resName, const string flag, TH1D* div1520, TH1D* div2025, TH1D* div2530, TH1D* div3040, TH1D* div4060, std::vector<TH1D*> hon, std::vector<TH1D*> hoff, TH2D* her, TH2D* p6dec) {
  string cName = (string) "cslice" + (string) raw->GetName();
  TCanvas *cslice = MakeCanvas(cName.c_str(), "0", 1200,800);
  ...
  vector<TH1D*> herXs = Projection2D(her, nBins, ranges, "x");
  vector<TH1D*> p6decXs = Projection2D(p6dec, nBins, ranges, "x");
  vector<TH1D*> clones;
  
  for (int i = 0; i < recoXs.size(); ++ i) {clones.push_back((TH1D*) recoXs[i]->Clone(("fully_corrected_" + to_string(i)).c_str()));};
...
*/
  ...
  if (log != "Y") {loy = 0; hiy = 7;}
  if (flag == "mass") {lox = -1; hix = -1; loy = 0; hiy = 0.8;} else if (log == "Y") {loy = 0.001; hiy = 50;} else {lox = 0; hix = 0.5;}
 
  cout << "data p6undec p8dec p8undec geant unfold herwig p6dec corr" << endl;
  
  for(int i = 0; i < nBins; ++ i) { 
    Prettify1DwLineStyle(genXs[i], kGreen, kSolid, 5, xTitle, yTitle,lox,hix,loy,hiy);
    Prettify1DwLineStyle(p6decXs[i], kGreen, kDashed, 5, xTitle, yTitle, lox,hix,loy,hiy);
    Prettify1DwLineStyle(hoff[i], kViolet, kSolid, 5, xTitle, yTitle,lox,hix,loy,hiy);
    Prettify1DwLineStyle(hon[i], kViolet, kDashed, 5, xTitle, yTitle,lox,hix,loy,hiy);
    Prettify1DwLineStyle(herXs[i], kCyan/*7*/, kDashed, 5, xTitle, yTitle, lox,hix,loy,hiy);
    Prettify1D(detXs[i], kBlue, kOpenCircle, 2, kBlue, xTitle, yTitle,lox,hix,loy,hiy);
    Prettify1D(rawXs[i], kBlack, kOpenStar, 2.5, kBlack, xTitle, yTitle,lox,hix,loy,hiy);
    Prettify1D(recoXs[i], kRed, kOpenStar, 2.5, kRed, xTitle, yTitle,lox,hix,loy,hiy);
    //Prettify1D(clones[i], kOrange, kFullStar, 3, kOrange, xTitle, yTitle, lox,hix,loy,hiy);
    recoXs[i]->SetTitle("");

    cout << rawXs[i]->Integral() << " " << genXs[i]->Integral() << " " << hon[i]->Integral() << " " << hoff[i]->Integral() << " " << detXs[i]->Integral() << " " << recoXs[i]->Integral() << " " << herXs[i]->Integral() << " " << p6decXs[i]->Integral() << " " << setprecision(5) << clones[i]->Integral() << endl;
  }

    
  if (flag == "mass") {
    clones[0]->Divide(div1520); clones[1]->Divide(div2025); clones[2]->Divide(div2530); clones[3]->Divide(div3040); clones[4]->Divide(div4060);
    for (int i = 0; i < nBins; ++ i) {
      Prettify1D(clones[i], kRed/*9*/, kFullStar, 2.5, kOrange, xTitle, yTitle, lox,hix,loy,hiy);
    }
  }  
  
  TLegend * tu = new TLegend(0.1,0.15,0.8,0.45); tu->SetBorderSize(0);
  tu->AddEntry(herXs[0],"HERWIG7 w/ decays","l");
  tu->AddEntry(hon[0],"PYTHIA8 w/ decays", "l");
  tu->AddEntry(hoff[0],"PYTHIA8 w/o decays","l");
  //  tu->AddEntry(p6decXs[0],"PYTHIA6 w/ decays","l");
  tu->AddEntry(genXs[0],"PYTHIA6 w/o decays","l");
  tu->AddEntry(detXs[0],"PYTHIA6+GEANT","p");
  tu->AddEntry(rawXs[0],"Raw data", "p");
  tu->AddEntry(recoXs[0],"Unfolded data (4 iter)","p");
  tu->AddEntry(clones[0],"Unfolded data corrected for decays","p");
  ...
  //  gStyle->SetPalette(109,0);//kCool palette


  cslice->cd(1); TLatex* title = PanelTitle(); tu->Draw("9");
  for (int i = 0; i < nBins; ++ i) {
    cslice->cd(i+2); /*p6decXs[i]->Draw("sameC");*/ herXs[i]->Draw("sameC9"); hon[i]->Draw("C,same9"); hoff[i]->Draw("C,same9"); genXs[i]->Draw("C,same9"); rawXs[i]->Draw("same9"); detXs[i]->Draw("same9"); clones[i]->Draw("same9");  recoXs[i]->Draw("same9"); tslices[i]->Draw("same9");
  }
 
  //cslice->SaveAs("~/jetmass/production/plots/decays_correction/unfolded_data_w_correction.pdf");
...
void MCClosure1D(TFile *File, const std::string xTitle, const std::string resName,const std::string out,const std::string filetype, TH1D* detOdd, TH1D* genOdd, TH1D* genEven) {
  ...
  */
  ...
  Prettify1D(reco_opp, kBlue, kOpenSquare, 2, kBlue, xTitle, "arb.",lox,hix,loy,hiy);
...
void MCClosure2DFull(TFile *File, const std::string xTitle, const std::string resName,const std::string out,const std::string filetype, TH2D* det, TH2D* gen_opp, TH2D* gen_same) {
  ...
  RooUnfoldResponse *res_odd = (RooUnfoldResponse*) File->Get((resName+"_odd").c_str());
  
  RooUnfoldBayes *unfolded_opp = new RooUnfoldBayes(res, det, 4, false, ("unfolded_4iter" + (string) det->GetName() + "_opp").c_str(),"");
  RooUnfoldBayes *unfolded_same = new RooUnfoldBayes(res_odd, det, 4, false, ("unfolded_4iter" + (string) det->GetName() + "_same").c_str(),"");
  
  TH2D *reco_opp = (TH2D*) unfolded_opp->Hreco((RooUnfold::ErrorTreatment) 3);
  TH2D *reco_same = (TH2D*) unfolded_same->Hreco((RooUnfold::ErrorTreatment) 3);
  
  const unsigned nBins = 1;
  double ranges[nBins+1] = {0,999};
  vector<TH1D*> recoYs = Projection2D(reco_opp, nBins, ranges, "y");
  vector<TH1D*> genYs = Projection2D(gen_opp, nBins, ranges, "y");
  vector<TH1D*> recoYs_same = Projection2D(reco_same, nBins, ranges, "y");
  vector<TH1D*> genYs_same = Projection2D(gen_same, nBins, ranges, "y");

  //vector<TH1D*> detYs = Projection2D(det, nBins, ranges, "y");
  
  recoYs[0]->Divide(genYs[0]); recoYs_same[0]->Divide(genYs_same[0]);
  
  double loy = 0.5; double hiy = 1.5; double lox = 0; double hix = 60;
  
  Prettify1D(recoYs[0], kBlue, kOpenSquare, 2, kBlue, xTitle, "arb.",lox,hix,loy,hiy);
  Prettify1D(recoYs_same[0], kViolet, kFullCircle, 2, kViolet, xTitle, "arb.",lox,hix,loy,hiy);
  recoYs[0]->SetTitle("STAR Simulation"); recoYs_same[0]->SetTitle("");
  ...
  l->AddEntry(recoYs_same[0],"Same side (4 iter.)","p");

  recoYs[0]->Draw(); recoYs_same[0]->Draw("same"); one->Draw("same"); low->Draw("same"); high->Draw("same");
  t->Draw("same"); l->Draw("same");
...
void MCClosure2DSlices(TFile *File, const std::string xTitle, const std::string resName,const std::string out,const std::string filetype, TH2D* det, TH2D* gen_opp, TH2D* gen_same) {
  TCanvas* cclosSlice = MakeCanvas(("cclosSlice" + resName).c_str(),"0",1400,1000); cclosSlice->cd();
  DivideCanvas(cclosSlice,"0",3,2);
  
  RooUnfoldResponse *res = (RooUnfoldResponse*) File->Get(resName.c_str());
  RooUnfoldResponse *res_same = (RooUnfoldResponse*) File->Get((resName+"_odd").c_str());
  
  RooUnfoldBayes *unfolded_opp = new RooUnfoldBayes(res, det, 4, false, ("unfolded_4iter" + (string) det->GetName() + "_opp").c_str(),"");
  RooUnfoldBayes *unfolded_same = new RooUnfoldBayes(res_same, det, 4, false, ("unfolded_4iter" + (string) det->GetName() + "_same").c_str(),"");
  ...
  vector<TH1D*> recoXs_same = Projection2D(reco_same, nBins, ranges, "x");
  vector<TH1D*> genXs_same = Projection2D(gen_same, nBins, ranges, "x");
  //vector<TH1D*> detYs = Projection2D(det, nBins, ranges_d, "y");

  for (int i = 0; i < recoXs.size(); ++ i) {
    recoXs[i]->Divide(genXs[i]); recoXs_same[i]->Divide(genXs_same[i]);
  }
  
  double loy = 0.5; double hiy = 1.5; double lox = 0; double hix = 0.5; 
  if (xTitle.find("p_{T}") != std::string::npos) {hix *= 120;} else if (xTitle.find("M") != std::string::npos) {hix *= 28;}
  
  for (int i = 0; i < recoXs.size(); ++ i) {
    Prettify1D(recoXs[i], kBlue, kOpenSquare, 2, kBlue, xTitle, "arb.",lox,hix,loy,hiy);
    Prettify1D(recoXs_same[i], kViolet, kFullCircle, 2, kViolet, xTitle, "arb.",lox,hix,loy,hiy);  
  }
  recoXs[0]->SetTitle("STAR Simulation"); recoXs_same[0]->SetTitle("");
  //
  TLine *one = new TLine(0,1,60,1); TLine *low = new TLine(0,0.95,60,0.95); TLine *high = new TLine(0,1.05,60,1.05); 
  one->SetLineStyle(kDashed); low->SetLineStyle(kDashed); high->SetLineStyle(kDashed);
  ...
  l->AddEntry(recoXs_same[0],"Same side (4 iter.)","p");
  ...
  for (int i = 0; i < recoXs.size(); ++ i) {
    cclosSlice->cd(i+2);
    recoXs[i]->Draw(); recoXs_same[i]->Draw("same"); one->Draw("same"); low->Draw("same"); high->Draw("same");
    tslices[i]->Draw("same");
  }
...
void unfolding () {
  string dir = "~/jetmass/";
  string other_sims = "~/jetmass/production/macros/";
  string matchin = "out/matching/";
  string closein = "out/closure/";
  string pyin = "out/sim/py/";
  string gein = "out/sim/ge/";
  string datain = "out/data/";
  /* string p8in = other_sims + "hists/ratio.root";
  string p6decin = other_sims + "p6_dec_hists/";
  string herin = other_sims + "herwig_hists/";*/
  string simsin = "production/macros/hists/";
  string in = "macros/hists/";
  string infile = "hists.root";
  string file = "test2.root";
  ...
  TFile* p8File = new TFile((dir+simsin+infile).c_str(),"READ");
  TFile* p6decFile = new TFile((dir+simsin+infile).c_str(),"READ");
  TFile* herFile = new TFile((dir+simsin+infile).c_str(),"READ");
  ...
  TH1D* div1520 = (TH1D*) p8File->Get("hratio1520");
  TH1D* div2025 = (TH1D*) p8File->Get("hratio2025");
  TH1D* div2530 = (TH1D*) p8File->Get("hratio2530");
  TH1D* div3040 = (TH1D*) p8File->Get("hratio3040");
  TH1D* div4060 = (TH1D*) p8File->Get("hratio4060");
  TH1D* hon1520 = (TH1D*) p8File->Get("hon1520");
  TH1D* hon2025 = (TH1D*) p8File->Get("hon2025");
  TH1D* hon2530 = (TH1D*) p8File->Get("hon2530");
  TH1D* hon3040 = (TH1D*) p8File->Get("hon3040");
  TH1D* hon4060 = (TH1D*) p8File->Get("hon4060");
  TH1D* hoff1520 = (TH1D*) p8File->Get("hoff1520");
  TH1D* hoff2025 = (TH1D*) p8File->Get("hoff2025");
  TH1D* hoff2530 = (TH1D*) p8File->Get("hoff2530");
  TH1D* hoff3040 = (TH1D*) p8File->Get("hoff3040");
  TH1D* hoff4060 = (TH1D*) p8File->Get("hoff4060");

  
  std::vector<TH1D*> hon = {hon1520,hon2025,hon2530,hon3040,hon4060};
  std::vector<TH1D*> hoff = {hoff1520,hoff2025,hoff2530,hoff3040,hoff4060};
  */
  //  TH2D* her2d = (TH2D*) herFile->Get("m_v_pt_her");
  //TH2D* p6dec2d = (TH2D*) p6decFile->Get("m_v_pt_dec");
  ...
  */
  ...
  //SliceUnfolded4D(matchFile, m2d[0], m2d[1], m2d[2], "0", out, filetype, "M [GeV/c^{2}]", "pt_m_response", "mass", div1520, div2025, div2530, div3040, div4060,hon,hoff, her2d, p6dec2d);
  ...
  */
  ...
  TH2D* genPtMSame = (TH2D*) closureFile->Get("pt_m_gen_odd"); TH2D* detPtM = (TH2D*) closureFile->Get("pt_m_det_odd");
  TH2D* genPtMOpp = (TH2D*) closureFile->Get("pt_m_gen_even");
  ...
  MCClosure1D(closureFile,"p_{T} [GeV/c]", "pt",closeout,filetype, detPt, genPtOpp, genPtSame);
  
  MCClosure2DFull(closureFile,"p_{T} [GeV/c]", "pt_m_response",closeout,filetype, detPtM, genPtMOpp, genPtMSame);
  ...
  MCClosure2DSlices(closureFile,"M [GeV/c^{2}]", "pt_m_response",closeout,filetype, detPtM, genPtMOpp, genPtMSame);
  ...
  //  genPtMSame->ProjectionX("mgen",genPtMSame->GetYaxis()->FindBin(15),genPtMSame->GetYaxis()->FindBin(20))->Draw();
  //detPtM->ProjectionX("mdet",detPtM->GetYaxis()->FindBin(15),genPtMSame->GetYaxis()->FindBin(20))->Draw("same");
